...
- **`context_update` cwd-match must not overwrite pane-bound sessions when incoming paneId is absent**: `findSessionByCwd` in `useAgentEvents.subagentReducers.ts` skips any session with `paneId` set when the incoming action carries no `paneId`. Reason: the IDE runs inside itself — the terminal Claude session shares `cwd` with the in-app workbench pane session, and without this guard the terminal's higher token count flickers into the pane's context gauge.
