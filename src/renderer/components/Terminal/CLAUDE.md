...
- **DOM renderer is the sole renderer** (current): `@xterm/addon-webgl` is installed but NOT loaded. The DOM renderer is xterm's built-in default when no accelerated renderer addon is active. WebGL was dropped because xterm's WebGL renderer composites the canvas fully opaque regardless of `allowTransparency` (upstream issue xtermjs/xterm.js#1004, open since 2018) — the GPU's `getContext()` call omits `alpha:true`, so the well-theme tinted transparency never showed through. The DOM renderer honours `allowTransparency` correctly, enabling the tinted-well/glass aesthetic for Modern and similar themes. `webglAddonRef`/`webglFailedRef` refs and the `loadWebGLAddon` function have been removed. Package removal + patch cleanup tracked in `roadmap/follow-ups/2026-05-21-remove-xterm-webgl-dependency.md`. Addon load order is still centralised in `terminalAddonManifest.ts` (`loadOrder: 'pre-open' | 'post-open'`).
...
- **Xterm canvas bg — tinted for well themes, opaque for unset** (current): `buildXtermTheme()` reads `--term-canvas-bg` first; that var is written by `applyComponentTokens` ONLY when the active theme declares a `terminalWell` (e.g. Modern: `rgba(6,8,16,0.1)`). For themes without a well (cursor, kiro, light, high-contrast) the var is absent → falls back to `--palette-term-bg` (`#0c0c0e`, opaque) — byte-for-byte identical to prior behaviour. `allowTransparency:true` now works correctly because the DOM renderer (sole renderer since WebGL was dropped) honours the flag — WebGL ignored it at the GPU level. The `CONTAINER_STYLE.backgroundColor` in `TerminalInstanceView.helpers.ts` uses the same `var(--term-canvas-bg, var(--palette-term-bg, #0c0c0e))` chain so the gap strip behind the canvas matches the canvas colour for both well and non-well themes. RISK: Claude TUI fill-bg cells may composite against Mica with a translucent canvas; revert `allowTransparency` to `false` if live verification shows broken status boxes (Wave 95 ADR D4). `cursorAccent` always uses `--palette-term-bg` directly (opaque) so the cursor interior block colour is never semi-transparent.
- **Tinted well must sync on mount, not only on theme-change events** (2026-05-22): `useThemeSync` (`TerminalInstanceUiState.ts`) calls `syncTheme()` once on mount in addition to listening for `agent-ide:theme-applied`. Reason: the canon workbench auto-spawns its terminals immediately on mount, which can win the race against async theme hydration (`hydrateThemeOnMount` awaits an IPC round-trip). When it does, xterm initialises with the opaque `#0d0d0d` fallback (because `--term-canvas-bg` isn't set yet) and the `agent-ide:theme-applied` event fires *before* the listener's `useEffect` attaches — so without the mount sync the tinted well never paints and Modern/Warp/Retro terminals stay opaque black. Legacy terminals never hit this: they only mount on user action, well after hydration. See `TerminalInstanceUiState.test.ts`.
...
- **WebGL atlas-merge patch — dormant** (historical): `@xterm/addon-webgl` 0.19.0 had a documented atlas-corruption bug (#5847) patched locally via `patches/addon-webgl-0.19.0.patched.{mjs,js}` + `tools/apply-patches.mjs`. Now that the WebGL addon is not loaded, the patch is dormant (postinstall still applies it but it has no runtime effect). Full removal of the package, patches, and `tools/apply-patches.mjs` WebGL step is tracked in `roadmap/follow-ups/2026-05-21-remove-xterm-webgl-dependency.md`.
