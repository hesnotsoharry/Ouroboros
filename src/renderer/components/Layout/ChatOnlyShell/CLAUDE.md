...
- **Wave 99**: Agent-completion rail indicators. Surfaces agent finish/error on three rail tiers (outer project dot, inner-rail terminals list, dock terminal tabs). See the **Agent-completion indicators** section below.

## Agent-completion indicators (Wave 99)

Shows when a Claude Code agent finishes (green = `complete`, red = `error`) on the workbench rail tiers. **Source of truth is the live `AgentSession` store, NOT chat threads.**

- **`useAgentCompletionIndicators(projects)`** (`src/renderer/hooks/`) is the single derivation hook: reads `useAgentEventsContext().agents`, joins by `cwd` (project) and by `AgentSession.id` (= a terminal's `claudeSessionId`). Returns `statusByProject` + `statusByClaudeSessionId` + `markProjectViewed` / `markSessionViewed`.
- **One shared instance** is mounted via `AgentCompletionIndicatorsContext` (provider at `ChatWorkbenchBody`) so the outer rail, the inner terminals list, and the dock tabs share ONE viewed-state. Mounting the hook per-surface would diverge the watermarks.
- **Two independent viewed watermarks** (`lastProjectViewedAt` keyed by normalized project path, `lastSessionViewedAt` keyed by session id). `markProjectViewed` clears ONLY the outer dot — it must NOT clear the per-terminal dots, or they'd vanish the instant you click into the project (the "see which terminal finished" goal). Per-terminal dots clear on terminal focus/select via `markSessionViewed`. **Reason:** a single watermark made project-select wipe the terminal indicators before the user could see them.
- **Surfaces:** outer dot in `OuterProjectRail` (`OuterProjectRail.dot.tsx`); shared `CompletionDot.tsx` on `DockSlotTabs` tabs and `InnerSidebarTerminals` rows.
- **Attention-source repair (ADR 6):** `useWorkbenchAttention` gained an additive `agentStatusBySessionRecordId` input. Before Wave 99, `live`/`completed-unseen`/`failed` derived only from `AgentChatThreadRecord.status` (the retired chat path) — dead for terminal sessions. The new branch derives them from `AgentSession.status`, reviving the **Live** chip for terminal runs. The legacy chat-thread path is kept as fallback. A threadless session now derives purely from agent status (fixes the `null === null → completed-unseen` quirk in `deriveAttention`). The session-row chip lands on `InnerSidebarChats`, which is currently behind a disabled `chats` tab (`InnerSidebar.tsx`) — wiring is correct but dormant until that tab returns; the visible inner indicator is the terminals list + dock tabs.
- **Gotcha — terminal↔session binding is heuristic.** The per-terminal dot relies on `useClaudeSessionCapture` (`useTerminalSessions.sync.ts`) which can bind a background-launched `claude` to the wrong tab. The outer project dot (cwd-based) is the reliable signal and does not depend on this binding.
