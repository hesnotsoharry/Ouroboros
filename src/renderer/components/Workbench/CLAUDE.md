...
- **AgentGlobe consumes `useAgentEventsContext` (Wave 3+) and `useWorkbenchTabsContextSafe` (pane-aware fix).** `TitleBar/AgentGlobe.tsx` → `useWorkbenchGlobeData` → `useAgentEventsContext()`, which **throws** outside an `AgentEventsProvider`. Any test rendering `<Workbench />` (or `<TitleBar />`) must mock `useAgentEventsContext`. Additionally, any test that mocks `./Terminals/WorkbenchTabsProvider` and renders `AgentGlobe` must include `useWorkbenchTabsContextSafe: vi.fn().mockReturnValue(null)` in the mock — omitting it crashes because the globe imports the function from that module. See `Workbench.test.tsx` for the correct mock shape. Reason: the pane-aware fix added `useWorkbenchTabsContextSafe` so the globe resolves the active pane id without throwing in test isolation.
- **AgentGlobe pane-aware binding (pane-unaware globe fix).** `useWorkbenchGlobeData` now derives the active pane id via `useActiveWorkbenchFrame → useWorkbenchTabsContextSafe → activeTab.id`, then finds the session whose `session.paneId` matches. When paneId is null (outside provider) OR no session matches the pane (tab spawned, claude not started), it falls back to `selectPrimarySession` (global most-recently-active, internal excluded). The old code always used `selectPrimarySession` globally, which caused the globe to lock onto the outer/ambient session when ~100 ambient sessions were in the pool.
...
- **Tab state is a SINGLETON owned by `WorkbenchTabsProvider`; `useWorkbenchTabs` is now a thin wrapper over `useWorkbenchTabsContext(frame)` (2026-05-30 bug fix).** Reason: `useWorkbenchTabs` used to own a per-call `useState` tab collection AND spawn a claude on mount, so calling it from BOTH `TerminalShell` (line ~84) and `AgentSidebar`'s `useActivePaneId` created two independent collections + two spawns — the sidebar bound to its own idle copy's `activeTabId` (never the tab the user typed in), and the workbench startup-double-spawned claude. The provider (mounted in `Workbench.tsx` under `key={projectKey}`, above both `CenterPane` and `AgentSidebar`) holds ONE collection per frame + ONE shared `spawnedTabsRef` so a tab spawns at most once. Do NOT re-introduce a second `useState`-backed instance of the tab machine — any new consumer reads `useWorkbenchTabsContext(frame)`. Root cause + fix plan: `roadmap/bugs/2026-05-30-workbench-tab-state-instance-split.md`. (Known pre-existing follow-ups surfaced during this fix, NOT addressed here: `session_stop` un-owns sessions in `src/main/hooks.ts` → multi-turn tool events dropped; project-switch remount respawns + orphans the live session. AgentGlobe `selectPrimarySession` pane-unaware issue: FIXED in the pane-aware globe fix — see "AgentGlobe pane-aware binding" gotcha above.)
- **`WorkbenchTabsProvider` is the SOLE spawn authority; cold-start always spawns FRESH, never `--resume` (2026-05-30 freeze-fix).** `useWorkbenchTerminals` is now id-only — it returns stable fallback ids but NEVER calls `pty.spawn` or `pty.spawnClaude`. Removing spawn from there eliminated: (a) double-spawn (one orphaned pty from the old hook + one from the provider), and (b) the bogus `--resume` flag (resumeMode was set to a pane-id string, not a real Claude session id → the session picker appeared on every cold start). The provider spawns fresh via `spawnTab()` — no `resumeMode` ever passed. The restored tab collection's structure (label, id) is preserved across sessions; only the CC process itself starts fresh.
- **Cold-start spawn id MUST equal the displayed `activeTabId` (id-match invariant, 2026-05-30).** In `WorkbenchTabsProvider`'s `useTabRestoreInit`, the cold-start else-branch must spawn the tab ALREADY in `collection` state (from `useState` init) — NOT a freshly-built tab. Building a new tab inside the effect produces a different id from what `useWorkbenchTabsContext` returns as `activeTabId` → blank terminal (PTY sends data to id nobody listens to). The fix uses `collectionRef.current.tabs[0]` (latest value via ref) and the `useProjectSwitch` effect skips the `setUpperCollection(makeDefaultCollection)` reset on first mount (prev===undefined) since `useState` already initialized to the correct collection. This invariant is guarded by `WorkbenchTabsProvider.idmatch.acceptance.test.tsx`.
- **CC tabs do NOT auto-spawn — gated behind the "Start Claude" button (2026-05-31).** Opening or switching to a project no longer auto-spawns a Claude (`cc`) PTY (it burned tokens on sessions the user never used). In `useTabRestoreInit`, the cold-start branch is `kind === 'shell'`-gated and the restored path calls `spawnRestoredShellTabs` (cc excluded) — cc tabs are created in the collection but left UNSPAWNED. `WorkbenchTabsProvider` exposes `spawnedTabIds: ReadonlySet<string>` (reactive `spawnedTabIdsState`, a UI-re-render mirror of the `spawnedTabsRef` dedup authority) and `spawnCcTab(tabId)`. `TerminalShell` renders `<StartClaudePrompt>` (the "Start Claude" button) instead of `<TerminalWell>` when `kind === 'cc' && activeTabId !== null && !spawnedTabIds.has(activeTabId)`; the button calls `spawnCcTab(activeTabId)` → fresh `spawnTab(id, 'cc', cwd)`. Already-started panes stay started in-session (the cached/reconnect path never clears `spawnedTabIds`); on app restart `spawnedTabIds` resets empty so every cc pane shows the button again. This SUPERSEDES the two "cold-start always spawns FRESH" gotchas above FOR cc tabs (they remain accurate for shell tabs and for what happens once the user clicks Start Claude). Gated by `TerminalShell.addTab.acceptance.test.tsx` + `WorkbenchTabsProvider.idmatch.acceptance.test.tsx`. Cole decision 2026-05-31.
