...
const BG: Record<string, string> = { add: 'var(--success-tint)', del: 'var(--error-tint)', ctx: 'transparent' };
const SIGN: Record<string, string> = { add: '+', del: '−', ctx: ' ' };
const SIGN_COLOR: Record<string, string> = { add: 'var(--success)', del: 'var(--error)', ctx: 'var(--ink-4)' };

interface DiffRowProps { line: MockDiffLine }

function DiffRow({ line }: DiffRowProps): React.ReactElement {
  return (
    <div style={{ display: 'flex', background: BG[line.type], fontFamily: 'var(--font-mono, monospace)', fontSize: 11 }}>
      <span style={{ minWidth: 28, paddingRight: 4, textAlign: 'right', flexShrink: 0, color: 'var(--ink-4)', userSelect: 'none' }}>
        {line.n}
      </span>
      <span style={{ width: 12, flexShrink: 0, color: SIGN_COLOR[line.type], userSelect: 'none' }}>
        {SIGN[line.type]}
      </span>
      <span style={{ flex: 1, color: 'var(--ink-2)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'pre', paddingLeft: 4 }}>
        {line.text}
      </span>
    </div>
  );
...
interface StubButtonProps { label: string; variant: 'accent' | 'ghost'; icon?: React.ReactElement }
...
function StubButton({ label, variant, icon }: StubButtonProps): React.ReactElement {
  ...
  return (
    <button type="button" onClick={undefined} style={{
      ...
      padding: '3px 8px', borderRadius: 4, fontSize: 11, cursor: 'default',
...
interface HunkHeaderProps { file: string; startLine: number }
...
function HunkHeader({ file, startLine }: HunkHeaderProps): React.ReactElement {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 12px 4px' }}>
      ...
      <span style={{
        fontSize: 11, fontFamily: 'var(--font-mono, monospace)', color: 'var(--ink-4)',
        overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 160,
      }}>
        {filename}<span style={{ marginLeft: 4 }}>:{startLine}</span>
      </span>
    </div>
  );
}
...
interface LatestHunkProps { hunk?: MockDiffHunk }
...
export function LatestHunk({ hunk = MOCK_DIFF_HUNK_META }: LatestHunkProps): React.ReactElement {
  return (
    <div data-testid="latest-hunk" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{
        margin: '0 8px', borderRadius: 4, overflow: 'hidden',
        border: '1px solid var(--stroke-inner)', background: 'var(--glass-panel)',
      }}>
