...
- **IDE-spawned `claude` utility processes MUST suppress hooks**: any internal `claude` invocation the app spawns (e.g. `claudeUsagePoller.ts`'s `& claude` for `/usage`, and any summarizer / CLAUDE.md generator) inherits the IDE's hook env (`OUROBOROS_HOOKS_ADDRESS`/`TOKEN`), so once the workspace is trusted its `session_start` hook fires and registers a session that shows in the globe/sidebar as "running"/"thinking" — and never clears (there's no `session_stop` hook; see follow-up). Set `env: { ...process.env, OUROBOROS_CHAT_SESSION: '1' }` on the PTY/child spawn — both `session_start.mjs` and `agent_end.mjs` early-`exit(0)` on that flag, so no event is sent. (`OUROBOROS_INTERNAL=1` is NOT enough — it still sends the event with `internal:true`; the globe filters those via `selectPrimarySession` but that's defense-in-depth, not suppression.)
- **`tapDiffReview` gates on `paneId` before any async git work**: global hook installation means external Claude sessions on the same machine send hook events into the IDE's named pipe. `tapDiffReview` (`hooksDiffReview.ts`) must check `payload.paneId` and return early for sessions without it — otherwise every write-class tool event from any Claude process triggers `git rev-parse HEAD` against an unknown cwd, saturating CPU/disk. Do NOT remove or weaken this gate. The renderer's `ownedIds` check is a second layer, not a substitute — it runs after the expensive git work is already queued.
