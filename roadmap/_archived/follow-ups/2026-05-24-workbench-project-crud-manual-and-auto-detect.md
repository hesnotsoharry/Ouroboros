---
status: RESOLVED
resolved-during: wave-12-terminal-and-project-crud-chrome
severity: HIGH
created: 2026-05-24
updated: 2026-05-24
...
## Resolution (wave-12-terminal-and-project-crud-chrome)

Closed by `haiku-followup-auditor` during wave audit on 2026-05-24.

**Evidence:** Wave 12 Phase 2 (commit 2489f165) wires manual remove affordance (inline X button) to all three project-switcher surfaces (ProjectRail.tsx, TitleBarProjectDropdown.tsx, InnerRailProjectDropdown.tsx). Phase 1 (commit 48a0cfe6) adds `files.pathExists` IPC. Phase 2 extends `useWorkbenchProjects` to derive `exists: false` for stale paths and render them at 0.5 opacity. ADR D2 finalizes stale-detection UX (inline dim + always-visible X). Gap 1 (auto-detect) and Gap 2 (manual remove) both closed. Cole's verbatim complaint ("no way to remove projects currently") resolved by the inline X affordance across all three surfaces.
