...
- **Responsive breakpoint hook** — new `Workbench/useWorkbenchBreakpoint.ts` returning a 3-mode enum (`full` | `compact` | `unified`) from two `matchMedia` queries (1760px full↔compact, 1440px compact↔unified — below 1440 is uniformly unified once the HUD is dropped), SSR/StrictMode-safe.
...
- [ ] `Workbench/useWorkbenchBreakpoint.ts` returns `full`/`compact`/`unified` from the 1760 and 1440 `matchMedia` boundaries (≥1760 full; 1440–1759 compact; `<1440` unified — covering canon's 1180–1439 Unified band and the <1180 clamp), and updates reactively on the matchMedia `change` event; StrictMode-safe (no leaked listener).
