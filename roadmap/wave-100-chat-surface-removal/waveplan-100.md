...
## ⚠️ SCOPE CORRECTION (2026-05-19, re-verified 2026-05-27) — read before executing any phase

Two corrections landed after the original Phase 0 / Phase A:

1. **`src/renderer/components/Layout/ChatOnlyShell/` is NOT chat UI — DO NOT DELETE IT.** It is the live "chat workbench shell" (Wave 89+ terminal-first); `App.helpers.tsx:263` mounts it for chat-windows, the immersive flag, **and all mobile web** (`isMobileWeb` is locked to it), passing `terminal={hooks.terminal}` — it hosts terminal sessions. The "ChatOnlyShell" name is a historical artifact (Wave 42 "chat-only shell" → Wave 89 terminal workbench). **Phase B must NOT delete `ChatOnlyShell/` and must NOT strip the App.helpers immersive branch.** The renderer cut shrinks to the genuinely-dead `AgentChat/` *conversation* components only, plus any `ChatOnlyShell` files made dead by Wave 99's `useWorkbenchAttention` rewire (audit in Phase B pre-work).
2. **Wave 99 (rail) SHIPPED 2026-05-26 — sequencing blocker CLEARED.** `useWorkbenchAttention` now consumes `agentStatusBySessionRecordId` derived from `AgentSession.status`, not `AgentChatThreadRecord.status`. Wave 100 is unblocked.

**Re-verification (2026-05-27):** `wave-100-phase0-reverify-2026-05-27.md` re-grounds every line-number cite + adds material divergences against master HEAD `1d676d7a` (180 commits drifted since 2026-05-19, including Waves 19-22). Phase A grew from 3 to 7 items; `contextLayer/` shrank 58 → 34 files; two new IPC handlers surfaced as unlisted CUT targets; one Phase D ordering risk. The phase tables below have been updated to match.

The original Phase A commit on the abandoned `wave-100-chat-surface-removal` local branch is discarded — Wave 100 starts fresh from master per the 2026-05-27 fresh-start decision.

## Status

PLANNED · target v2.35.0 · drafted 2026-05-19, re-verified 2026-05-27. Wave 99 blocker cleared. Ready to execute Phase A on a fresh worktree from master HEAD `1d676d7a`.
...
7. **Semver: minor `v2.35.0`** — removal of a whole (already-dead) feature surface with no external/public API contract break. Per the development-pipeline semver-judgment grant. RESOLVED.

## Scope

**In scope:**

- Remove `src/renderer/components/AgentChat/` (and `src/renderer/components/Orchestration/` — ranker dashboard, see Phase 0 Check (b)). Per SCOPE CORRECTION: KEEP `src/renderer/components/Layout/ChatOnlyShell/` (live workbench + mobile-web shell) and KEEP the `isImmersive` / `ChatOnlyShellWrapper` branch in `App.helpers.tsx`. Phase B audits `ChatOnlyShell/*` and deletes only files made dead by AgentChat removal + Wave 99's `useWorkbenchAttention` rewire. Remove `src/renderer/types/electron-agent-chat.d.ts` and its re-export from `electron.d.ts`.
...
- CHANGELOG `[2.35.0]`, version bump, local tag `v2.35.0` (no push per bulletin).
...
| A | Relocate live helpers off chat modules (7 items, expanded 2026-05-27) | `sonnet-implementer` | **A1:** Migrate 11 surviving IPC handlers' `getErrorMessage` import from `agentChat/utils` to existing `src/main/utils.ts` (handlers: `approvalHandlers`, `costHandlers`, `crashHandlers`, `extensionStore`, `mcpStore`, `mcp`, `sessions`, `shellHistoryHandlers`, `symbolHandlers`, `updaterHandlers`, `usageHandlers`). The other 2 (`agentChatFork`, `agentChatMerge`) get cut in Phase C. **A2:** Update `hooksSkillExecutionTap.ts:15` to import `ActiveStreamContext` from existing `src/main/hooks/types.ts` (already relocated by Wave 99). **A3:** Relocate `resolveClaudeCliSettings`/`resolveCodexCliSettings` out of `agentChat/settingsResolver.ts`; update `configSchemaTail.ts:7` to use `configDefaults.ts` constants for the fallback. **A4:** Relocate `captureHeadHash`/`createCheckpointCommit` from `agentChat/chatOrchestrationBridgeGit.ts` (consumer: `ipc-handlers/checkpoint.ts:23`) — inline into `checkpoint.ts` or new `src/main/gitHelpers.ts`. **A5 (Decision 9):** REMOVE `thread://` deep-link handling from `protocolHandler.ts:15` entirely (`parsePermalink`/`buildPermalink` go away with `agentChat/permalinks.ts` in Phase D — no chat-thread target to deep-link to post-Wave-100). **A6:** Update `hooks/types.ts:15` to import from `@shared/types/agentChat` instead of `../agentChat/types`. **A7:** Flag (do not fix) — `AgentChatThreadStore` type leak in `session/softDeleteGc.ts:10` and `session/sessionStartup.ts:31` (inline `import()`); Phase D resolves. Behavior-preserving. Gate: `tsc.node` + `tsc.web` + `test:main`/`test:ipc` scoped. |
| B | Remove renderer chat surface — `AgentChat/` only + `ChatOnlyShell` dead-file audit (rewritten 2026-05-27 per SCOPE CORRECTION) | `sonnet-implementer` | Delete `src/renderer/components/AgentChat/` (100+ files). **DO NOT delete `Layout/ChatOnlyShell/`** (live workbench + mobile-web shell per SCOPE CORRECTION). **DO NOT strip the App.helpers immersive branch** (mobile-web depends on it via `isMobileWeb`). Phase B pre-work: audit `ChatOnlyShell/*` imports from `AgentChat/` — delete only the specific `ChatOnlyShell/` files made dead by (a) AgentChat removal AND (b) Wave 99's `useWorkbenchAttention` rewire (e.g., the legacy `AgentChatThreadRecord` fallback path at `useWorkbenchAttention.ts:4` and consumers of it). Also delete `src/renderer/components/Orchestration/` (ranker dashboard — `wave-100-decisions.md` Check (b)). Remove `electron-agent-chat.d.ts` + its `electron.d.ts:27` re-export. Remove the stale "Wave 90 home for interactive claude" aspirational comment from `Layout/ChatOnlyShell/DockSlot.tsx:6-7`. UI-bearing (`Layout/**`) → smoke gate fires; `sonnet-phase-reviewer` pass (renderer shell boot path; verify ChatOnlyShell + mobile-web paths still mount). Gate: `tsc.web` + `test:renderer`/`test:layout` scoped + app boots into terminal workbench. |
...
| D | Remove agentChat main module + Wave 86 files + ordering-risk cleanup + config/shutdown fixes (expanded 2026-05-27) | `sonnet-implementer` | Delete `src/main/agentChat/` except the must-keep set; delete the 6 Wave 86 files. **S4 ordering risk (added 2026-05-27):** ALSO delete `src/main/orchestration/contextOutcomeObserverResearch.ts` in this phase — it imports `registerRevertListener` from `agentChat/chatOrchestrationBridgeGit.ts`; leaving the import live across Phase D → Phase F would break `tsc` between phases. **S5 implicit cut (added 2026-05-27):** ALSO delete `src/main/hooksShadowTap.ts` + `hooksShadowTap.test.ts` (becomes a permanent no-op once Phase C cuts its initializer in `chatStateNewPath.ts`); remove the `tapShadowPath` call from `hooksTapRunner.ts`. **A7 cleanup:** resolve `AgentChatThreadStore` type leaks in `session/softDeleteGc.ts:10` and `session/sessionStartup.ts:31` (inline `import()`). Fix `mainShutdown.ts` (`closeThreadStore`), `configSchemaTail.ts`/`configAppTypes.ts` (chat settings constants/types). Boundary (startup/config) → `sonnet-phase-reviewer` pass. Gate: `tsc.node` + `test:main` scoped + IDE relaunches clean + terminal session spawns. |
...
| F | Remove context-intelligence subsystem + contextLayer + startup/hook wiring (updated 2026-05-27) | `sonnet-implementer` | Delete the packet/selector/ranker/classifier/signal/writers/observer/retrain/worker/telemetry modules + `tools/train-context.py`. **NOTE (2026-05-27):** `graphSummaryBuilder.ts` already deleted by prior wave — skip. Remove hook taps in `hooksTapRunner.ts`; remove `initDecisionWriter`/`initOutcomeWriter`/`startContextRetrainTriggerIfEnabled`/`loadPersistedContextCache`/`startContextRefreshTimer` from `main.ts` + matching shutdown calls. **NEW CUT target (added 2026-05-27):** `src/main/ipc-handlers/contextRankerDashboardHandlers.ts` (registers live `context:getRankerDashboard` channel; imports `contextClassifier.ts` + `contextRetrainStartup.ts`); unwire from `ipc-handlers/index.ts:23-26`. **Per ADR Decision 8 (full cut):** delete `src/main/contextLayer/` dir (**34 files at HEAD, not 58** — Wave 22 shrank it), `repoIndexer*` + `lspDiagnosticsProvider` (now orphaned — CUT), and `registerOrchestrationStubHandlers` + the `orchestration:previewContext`/`buildContextPacket` channels (`ipc.ts:184-219`). **Already partially done by Wave 22:** `main.ts:173-180` contains a gutted `startContextLayerAsync` stub — Phase F removes the stub function + its call at `main.ts:235`. `mainStartupContextLayerTrigger.ts` already absent at HEAD — skip. Unwire the **8 remaining contextLayer hook call sites** (line numbers per re-verify doc): `hooksSessionHandlers.ts:102, 114`, `hooksLifecycleHandlers.ts:106 (whole fn), :119`, `ipc-handlers/filesHelpers.ts:181`, `ipc-handlers/gitOperations.ts:227`, `ipc-handlers/config.ts:131-143` (whole branch). Remove orphaned `@shared/types/orchestration*` + `electron-orchestration.d.ts` + the `orchestration` prop on `ElectronAPI` (Phase 0 Check b: no terminal consumer). **Largest phase — split mid-flight** (contextLayer dir / repoIndexer cluster / orchestration-types / contextRankerDashboard as sub-commits). Boundary/risky → `sonnet-phase-reviewer` pass. Gate: `tsc` both + `test:main`/`test:hooks`/`test:codebasegraph` scoped + terminal-edit + subagent-tracking smoke + a Claude session start (exercises the unwired `onSessionStart` hook). |
| G | Remove auto-router subsystem (expanded 2026-05-27) | `sonnet-implementer` | Delete `src/main/router/`; remove `shadowRouteHookEvent` call in `hooks.ts:261`, `observeDatasetGrowth` in `main.ts:40,238`, `routerShadowDrainHandler` in `mainTelemetryHandlers.ts:11`, `routerSettings`/`routerLastRetrainCount` config keys (`configAppTypes.ts:151,153`, `configSchemaTail.ts:198,224`), `train-router.py` if present. Unwire `qualitySignalCollector` importers: `hooksSessionHandlers.ts:25`, `telemetry/hookEventsDrainHandler.ts:33`, `mainShutdown.ts:26`. **NEW CUT target (added 2026-05-27):** `src/main/ipc-handlers/routerStats.ts` (registers live `router:getStats` + `router:getQualitySignals` channels); unwire from `ipc-handlers/index.ts:52`. Consider whether to clean up legacy `router-decisions.jsonl` + `router-quality-signals.jsonl` data files in user data dir (or leave on disk — they're harmless once the readers are gone). Boundary (hook/startup) → `sonnet-phase-reviewer` pass. Gate: `tsc.node` + `test:main`/`test:hooks` scoped + terminal prompt submit works, no router errors in log. |
...
| J | Wave wrap — ship | `orchestrator` | Per `~/.claude/notes/wave-process.md` § Wave's final phase: full lint + full tsc (both) + full vitest + `/review` mechanical gap-check + CHANGELOG `[2.35.0]` + version bump + local tag `v2.35.0` (no push). Run `/promote-vendor-lessons 100` and `/audit-followups wave-100-chat-surface-removal`; update `HANDOFF.md` + temperature log. Confirm the archive tag is intact. |
...
- [ ] `src/renderer/components/AgentChat/` does not exist. `src/renderer/components/Layout/ChatOnlyShell/` retains only the live workbench + mobile-web files (per SCOPE CORRECTION); Phase B audit removed only files made dead by AgentChat removal + Wave 99 rewire.
- [ ] `src/renderer/components/Orchestration/` does not exist.
...
- [ ] `CHANGELOG.md` has a `[2.35.0]` entry; `package.json` version is `2.35.0`; local tag `v2.35.0` exists at the wave tip.
...
| B | IDE main window renders the terminal workbench shell; mobile web still routes through `ChatOnlyShellWrapper` | `App.helpers.tsx` immersive branch retained (KEEP per SCOPE CORRECTION) → desktop renders `InnerAppLayout`, mobile-web/immersive renders `ChatOnlyShellWrapper` with terminal sessions → both paths mount cleanly with `AgentChat/` deleted | Desktop opens into the terminal workbench; mobile-web opens into the ChatOnlyShell wrapper hosting terminal sessions; devtools console shows no missing-module or render errors on either path. |
...
test ! -d src/renderer/components/AgentChat && test ! -d src/renderer/components/Orchestration && echo "RENDERER-CHAT-GONE"
# ChatOnlyShell/ is RETAINED per SCOPE CORRECTION — it hosts mobile-web + immersive terminal sessions
test -d src/renderer/components/Layout/ChatOnlyShell && echo "CHATONLYSHELL-RETAINED"
...
12. **Phase J (wrap)** — orchestrator runs: `npm run lint` (full); `tsc` both; `npm test` (full vitest, background — ~17 min Windows-local); `/review` mechanical gap-check (PASS or FLAG-addressed); write `wave-100-result.md`; CHANGELOG `[2.35.0]`; bump `package.json` to `2.20.0`; commit wrap; `git tag v2.35.0`; `/promote-vendor-lessons 100`; `/audit-followups wave-100-chat-surface-removal`; update `HANDOFF.md` + temperature-log row; confirm `archive/chat-surface-2026-05-19` intact. **Do NOT push** (bulletin: GH Actions minutes held until 2026-06-01). **Gate to close:** all gates green AND HANDOFF current.
