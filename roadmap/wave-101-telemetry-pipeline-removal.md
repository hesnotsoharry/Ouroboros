...
- **ResearchSettings UI — RESOLVED (Phase 5b, `f6a87af5`).** User chose "remove now." Deleted the panel (`ResearchSettings.tsx` + `ResearchSettingsAdvanced` + Parts + tests, 6 files), the `'research'` Settings tab, the `researchSettings` electron-store schema key, and the `ResearchSettings`/`ResearchMode` types. Stale `researchSettings` keys in existing `config.json` are silently ignored (no top-level `additionalProperties: false`).
- **Data-tree cleanup — RESOLVED (2026-05-29).** User chose "delete now." Removed both `~/.ouroboros/telemetry/` (graph-usage/mcp-spawn-cost jsonl, queue/, processed/) and `AppData/Roaming/ouroboros/telemetry/` (722 MB `_stopgap-backup` telemetry.db, ~200 MB events-*.jsonl mirror, fresh 2.3 MB telemetry.db) — ~935 MB freed. Live `codebase-graph.db` (538 MB) verified untouched. Remaining tiny orphan `research-cache.db` (16 KB, parent dir) left in place pending user nod.
- **Doc drift (next doc sweep):** `roadmap/docs/data-model.md:237` still references `researchSettings` — stale after Phase 5b.
