...
|---|---|---|---|
| 0 | `/ui-smoke 10` catch-up (Wave 10's deferred obligation) | `orchestrator` | **Pre-implementation gate.** Dispatch `sonnet-smoke-runner` against running `npm run dev` instance with `layout.canonWorkbench` enabled. Smoke targets: see Scope §Phase 0. Append catch-up report to `roadmap/wave-10-project-scoped-state-foundation/wave-10-smoke-report.md` (don't overwrite the deferred entry — chain a `## 2026-05-24 Catch-up` section). Tier rules: any HIGH/CRITICAL finding → pause Wave 11, file follow-up, Cole call. LOW/MEDIUM findings → log in catch-up report, proceed to Phase 1, surface at wrap. Test shape: **n/a — smoke only.** |
